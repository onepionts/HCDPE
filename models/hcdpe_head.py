...
class HCDPE_Head(TopdownHeatmapBaseHead):
    ...
    def forward(self, x, extra_x, joints=None, train=True):
        ...
        if self.stage_hcdpe == "classifier":
            ...
            cls_feat,cls_feat_h1,cls_feat_h2,non_causal_feat  = self.MHTrans(cls_feat,cls_feat_h1,cls_feat_h2)
            stable = False
            # 二维特征图拉直成一维
            ...
            if train:
                if stable:
                    weight_stable1 = weight_learner(cls_feat,non_causal_feat)
                    weight_stable_h1 = weight_learner(cls_feat_h1,non_causal_feat)
                    weight_stable_h2 = weight_learner(cls_feat_h2,non_causal_feat)
                else:
